...
export const loader: LoaderFunction = async ({
  ...
  //Get house requests from db (Uncompleted)
  try {
    const houseRequests = await houseRequestModel
      .find({ completed: false })
      .populate("projectId", "title")
      .lean();

    if (houseRequests.length > 0) {
      const houseRequestsWithStringIds = houseRequests.map((item) => ({
        ...item,
      ...
      return Response.json({
        success: true,
        requests: houseRequestsWithStringIds,
        reponseId: Date.now(),
      });
    } else {
      return Response.json({
...
export const action: ActionFunction = async ({
  ...
  if (intent === "change") {
    try {
      await houseRequestModel.updateOne(
      ...
      <>
        <div className="w-full flex flex-col items-center justify-start p-4">
          <p className="font-normal text-xl text-center">
            {translations.confirmCompleteRequest[lang]}
          </p>
...
const AdminRequests = () => {
  const { requests, success } = useLoaderData<{
    responseId: number;
    success: boolean;
    requests: {
      date: string;
