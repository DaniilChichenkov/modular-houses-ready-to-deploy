...
export const loader: LoaderFunction = async ({
  ...
  try {
    const feedbacks = await feedbackModel.find({ red: true }).lean();
    if (feedbacks.length > 0) {
      ...
      return Response.json({
        success: true,
        redFeedbacks: feedbacksWithStringIds,
        responseId: Date.now(),
      });
    } else {
      return Response.json({
      ...
      <section className="w-full grid grid-cols-1 px-10 mt-10 relative z-0 pb-20">
        <div className="w-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-y-10 md:gap-x-5 items-start">
          {navigation.state === "idle" ? (
            <>
              {loaderData &&
              loaderData.redFeedbacks &&
              loaderData.redFeedbacks.length ? (
                <>
                  {loaderData.redFeedbacks.map((item) => (
