...
import useChangeContactStore from "~/stores/ChangeContactStore";
import useAdminInfoModalStore from "~/stores/AdminInfoModalStore";

const translations = {
  changeContact: {
    eng: "Change Contact",
    rus: "Изменить контакт",
    est: "Muuda kontakti",
    nor: "Endre kontakt",
  },
  teamMemberName: {
...
export const loader: LoaderFunction = async ({
  ...
  if (!contactId) {
    return json({ success: false, msg: "No contact id provided" });
  }

  //Find record in db
  try {
    await connectToDB();
    const contact = await memberModel.findOne({ _id: contactId }).lean();

    if (!contact) {
      return json({ success: false, msg: "Nothing was found" });
    }

    return {
      ...contact,
      languages: JSON.parse(contact.languages),
      _id: contactId,
    };
  } catch (error) {
...
export const action: ActionFunction = async ({
  ...
  try {
    await connectToDB();
    const memberToChange = await memberModel.findOne({ _id: contactId });

    if (!memberToChange) {
      throw new Error("No member found");
    }

    memberToChange.name = name as string;
    memberToChange.tel = tel as string;
    memberToChange.email = email as string;
    memberToChange.languages = languages as string;
    memberToChange.position = position as string;
...
const AdminNewContactRoute = () => {
  ...
  //Store
  const name = useChangeContactStore((state) => state.name);
  const tel = useChangeContactStore((state) => state.tel);
  const email = useChangeContactStore((state) => state.email);
  const languages = useChangeContactStore((state) => state.languages);
  const position = useChangeContactStore((state) => state.position);

  const setName = useChangeContactStore((state) => state.setName);
  const setTel = useChangeContactStore((state) => state.setTel);
  const setEmail = useChangeContactStore((state) => state.setEmail);
  const setPosition = useChangeContactStore((state) => state.setPoisition);
  const setInitialPosition = useChangeContactStore(
    (state) => state.setInitialPosition
  );
  const selectPositionLanguage = useChangeContactStore(
    (state) => state.selectPositionLanguage
  );
  const selectLanguage = useChangeContactStore((state) => state.selectLanguage);
  const unSelectLanguage = useChangeContactStore(
    (state) => state.unSelectLanguage
  );
  const setInitialLanguages = useChangeContactStore(
    (state) => state.setInitialLanguages
  );
  const resetForm = useChangeContactStore((state) => state.resetForm);
  ...
  const handleSubmit = () => {
    ...
    const { name, tel, email, position, languages } =
      useChangeContactStore.getState();
    ...
    //Check for errors
    if (errors.length) {
      showModal("Contact create error", "You must fill all inputs");
    ...
    fetcher.submit(formData, {
      ...
      method: "POST",
      action: `/admin/${lang}/contacts/${loaderData._id}/change`,
    });
    ...
    if (fetcher.state === "idle") {
      if (fetcher.data) {
        if (fetcher.data.success) {
          showModal(
            translations.changeResults[lang!],
  ...
  //Populate form state manager
  useEffect(() => {
    setName(loaderData.name);
    setEmail(loaderData.email);
    setTel(loaderData.tel);
    setInitialPosition(JSON.parse(loaderData.position));
    setInitialLanguages(loaderData.languages);
  }, [loaderData]);
  ...
  return (
    <div className="w-full flex flex-col items-center justify-start gap-y-1 md:w-8/12 md:mx-auto lg:w-4/12">
      ...
      <h2 className="text-2xl font-semibold text-gray-900 sm:text-3xl text-center">
        {translations.changeContact[lang!]}
      </h2>
