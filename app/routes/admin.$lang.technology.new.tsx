...
import { NewTechnologyHeader, NewTechnologyContent } from "~/components/admin";
...
export const action: ActionFunction = async ({
  ...
  try {
    ...
    //Firstly try to create record in db
    const createdTechnologyArticle = await technologyModel.create({
      title,
      content: JSON.stringify(content),
    });

    //If record creation went well - create path for technology article
    const technologyId = createdTechnologyArticle._id.toString();
    const technologyBaseDirPath = path.join(
      process.cwd(),
      "public",
      "technology",
      technologyId
    ...
    for (const [key, value] of Object.entries(collections)) {
      ...
      const imagesCollectionPath = path.join(technologyBaseDirPath, key);
    ...
    if (Object.entries(collections).length > 0) {
      createdTechnologyArticle.imagesFolder = technologyId;
    }

    await createdTechnologyArticle.save();
  } catch (error) {
...
const AdminTechnologyNewRoute = () => {
  ...
  //Form manager
  const resetForm = useNewTechnologyItemStore((state) => state.resetForm);
  ...
  const handleSubmit = async () => {
    ...
    const { title, content } = useNewTechnologyItemStore.getState();
    ...
    fetcher.submit(formData, {
      ...
      method: "POST",
      action: `/admin/${lang}/technology/new`,
    });
  };
...
export default AdminTechnologyNewRoute;
