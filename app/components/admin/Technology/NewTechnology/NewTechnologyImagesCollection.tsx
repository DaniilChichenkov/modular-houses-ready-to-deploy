...
const NewTechnologyImagesCollectionItem = ({
  url,
  uuid,
  parentUUid,
}: {
  ...
  const deleteFileFromImagesCollection = useNewTechnologyItemStore(
    (state) => state.deleteFileFromImagesCollection
  );
  const replaceImagesCollectionFile = useNewTechnologyItemStore(
...
const NewTechnologyImagesCollection = ({
  uuid,
  files,
}: {
  uuid: string;
  ...
  const addFilesToImagesCollection = useNewTechnologyItemStore(
    (state) => state.addFilesToImagesCollection
  );
  const deleteImagesCollection = useNewTechnologyItemStore(
        ...
        {(files &&
          ...
          files.map((item) => (
            <div className="w-full" key={item.uuid}>
              {/* Image component */}
              <NewTechnologyImagesCollectionItem
                uuid={item.uuid!}
                url={item.previewUrl!}
                parentUUid={uuid}
              />
...
export default NewTechnologyImagesCollection;
