...
const translations = {
  ...
  titleInput: {
    eng: "Title Input",
    rus: "Поле ввода заголовка",
    est: "Pealkirja sisestusväli",
    nor: "Inndatafelt for tittel",
  },
  enterTitleHere: {
    eng: "Enter title here",
    rus: "Введите заголовок здесь",
    est: "Sisesta pealkiri siia",
    nor: "Skriv inn tittel her",
  },
};

const ChangeGalleryTitle = () => {
  ...
  //Store
  const title = useChangeGalleryStore((state) => state.title.content);
  const titleLang = useChangeGalleryStore((state) => state.title.selectedLang);
  const setTitle = useChangeGalleryStore((state) => state.setTitle);
  const setTitleLang = useChangeGalleryStore(
    (state) => state.selectTitleLanguage
  );
    ...
    <div className="flex flex-col items-start justify-start gap-y-1 w-full col-start-1 col-span-1">
      ...
      <div className="w-full rounded-lg shadow-md px-4 py-8 flex flex-col gap-y-8">
        ...
        <input
          ...
          placeholder={`${translations.enterTitleHere[lang!]}...`}
...
export default ChangeGalleryTitle;
