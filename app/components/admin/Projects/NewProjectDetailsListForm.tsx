...
type NewProjectDetailsListItemInputProps = {
  uuid: string;
  parentUUid: string;
  selectedLang: string;
  content: {
...
const NewProjectDetailsListItemInput = ({
  uuid,
  parentUUid,
  selectedLang,
  content,
}: NewProjectDetailsListItemInputProps) => {
  //Languages
  type SupportedLanguages = "eng" | "rus" | "est" | "nor";
  const { lang } = useParams<{ lang: SupportedLanguages }>();

  const deleteFeaturesListItem = useNewProjectStore(
    (state) => state.deleteFeaturesListItem
  );
  const setFeaturesListItemInput = useNewProjectStore(
...
type NewProjectDetailsListFormProps = {
  listItems: {
    uuid: string;
    content: {
      eng: string | null;
...
const NewProjectDetailsListForm = ({
  uuid,
  listItems,
  selectedLang,
  title,
}: NewProjectDetailsListFormProps) => {
  //Languages
  type SupportedLanguages = "eng" | "rus" | "est" | "nor";
  const { lang } = useParams<{ lang: SupportedLanguages }>();
  ...
  const addFeaturesListItem = useNewProjectStore(
    (state) => state.addFeaturesListItem
  );
  const deleteFeaturesList = useNewProjectStore(
    (state) => state.deleteFeaturesList
  );
  const setFeaturesListTitle = useNewProjectStore(
    (state) => state.setFeaturesListTitle
  );
  const selectFeaturesListLang = useNewProjectStore(
  ...
  return (
    <div className="w-full rounded-lg shadow-md px-4 py-8 flex flex-col gap-y-8">
      ...
      <div className="w-full flex flex-col items-start mt-3 gap-y-5">
        ...
        {listItems
          ? listItems.map((item) => (
              <NewProjectDetailsListItemInput
                parentUUid={uuid}
                uuid={item.uuid}
                key={item.uuid}
...
const NewProjectDetailsList = () => {
  const featuresList = useNewProjectStore((state) => state.featuresList);
  const addFeaturesList = useNewProjectStore((state) => state.addFeaturesList);
...
export default NewProjectDetailsList;
