...
const NewProjectQuickDataForm = () => {
  ...
  //Form store
  const price = useNewProjectStore((state) => state.price);
  const title = useNewProjectStore((state) => state.title);
  const quickDesc = useNewProjectStore((state) => state.quickDesc);
  const numberOfRooms = useNewProjectStore((state) => state.numberOfRooms);
  const area = useNewProjectStore((state) => state.area);
  const floors = useNewProjectStore((state) => state.floors);

  const setPrice = useNewProjectStore((state) => state.setPrice);
  const setTitle = useNewProjectStore((state) => state.setTitle);
  const setNumberOfRooms = useNewProjectStore(
    (state) => state.setNumberOfRooms
  );
  const setArea = useNewProjectStore((state) => state.setArea);
  const setFloors = useNewProjectStore((state) => state.setFloors);
  const selectQuickDescriptionLanguage = useNewProjectStore(
    (state) => state.selectQuickDescriptionLanguage
  );
  const setQuickDescription = useNewProjectStore(
    (state) => state.setQuickDescription
  );
  const setMainImage = useNewProjectStore((state) => state.setMainImage);
  ...
  const handleImgSelection = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    //Create url and store it in state
    const url = URL.createObjectURL(file!);
    setImgPreviewUrl(url);

    //Store file in Zustand Store
    setMainImage(file!);
  };
  ...
  const handleImageRemove = () => {
    ...
    //Remove file from Zustand Store
    setMainImage(null);
  };

  //Change url or clean it when component will unmount
  useEffect(() => {
    return () => {
      ...
      <div className="w-full rounded-lg shadow-md px-4 py-8 flex flex-col gap-y-8">
        ...
        {imgPreviewUrl && (
          <div className="w-full flex flex-col justify-start items-center">
            ...
            {/* Actual image */}
            <img className="mt-5" src={imgPreviewUrl} alt="Preview" />
          </div>
        )}
...
export default NewProjectQuickDataForm;
